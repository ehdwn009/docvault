      ...
      <div className="min-h-0 flex-1 overflow-auto px-3 pb-4">
        {tab === 'stats' && stats && (
          <dl className="space-y-2 text-sm">
            {(
              ...
              <div key={label} className="flex justify-between rounded bg-zinc-900 px-3 py-2">
        ...
        {tab === 'tree' && (
          <div className="space-y-3">
            {treeUsers.map((u) => (
              <div key={u.id}>
                <h4 className="px-1 text-xs font-semibold text-zinc-400">
                  {u.displayName ?? u.username}
                  <span className="ml-1 text-zinc-600">({u.files.length}개)</span>
                </h4>
                {u.files.map((f) => (
                  <button
                    key={f.id}
...
function UsersTab({
  ...
  return (
    <div>
      {/* 계정 발급 */}
      <form onSubmit={create} className="space-y-2 rounded-md border border-zinc-800 p-3">
        <h4 className="text-xs font-semibold text-zinc-400">계정 발급</h4>
        <input
          value={username}
          onChange={(e) => setUsername(e.target.value)}
          placeholder="아이디 (영소문자·숫자·_)"
          className="w-full rounded-md border border-zinc-700 bg-zinc-800 px-2 py-1.5 text-sm text-zinc-100 outline-none focus:border-zinc-400"
        />
        <input
          value={password}
        ...
        <button
          ...
          className="w-full rounded-md bg-zinc-100 py-1.5 text-sm font-medium text-zinc-900 hover:bg-white disabled:opacity-40"
        >
          발급
        </button>
      </form>
      ...
      <div className="mt-3 space-y-2">
        {users.map((u) => (
          <div key={u.id} className="rounded-md bg-zinc-900 px-3 py-2">
            <div className="flex items-center gap-2">
              <span className="text-sm text-zinc-100">{u.username}</span>
            ...
            {u.id !== meId && (
              <div className="mt-1.5 flex flex-wrap gap-2 text-[11px]">
                ...
                <button
                  onClick={() => {
                    const pw = window.prompt('새 비밀번호 (8자 이상)');
                    if (pw && pw.length >= 8)
                      void run(() =>
                        api(`/admin/users/${u.id}`, {
                          method: 'PUT',
                ...
                <button
                  onClick={() => {
                    if (window.confirm(`${u.username} 계정과 모든 데이터를 삭제할까요?`))
