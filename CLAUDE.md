...
- 스키마에 칸을 추가·변경하면 ERD(`docs/design/ERD_*.md`)의 **엔티티 블록과 관계 설명 양쪽**을 같은 작업에서 갱신한다 — 같은 사실이 두 군데 적혀 있어서, 한쪽만 고치면 반드시 어긋난다.
...
## 학습자료 (docs/data_ignored/학습자료/)

사용자는 코딩 입문자이며, 이 프로젝트를 교재 삼아 `docs/data_ignored/학습자료/`에 학습자료를 키우고 있다.

**이 폴더는 gitignore 대상이다** — 개인 자료이므로 커밋하지 않고, 커밋을 시도하지도 않는다. (git status에 안 잡히는 게 정상)

구성은 0부 오리엔테이션 / 1부 기초소양 / 2부 기획과설계 / 3부 개발환경 / 4부 언어마스터 / 5부 구조 / 6부 코드정독 / 7부 배포와운영 / 8부 성장 / 9부 보안심화 / 부록, 그리고 교재에서 갈라져 나온 책 3권(바이브코딩보안 · 혼자 운영하기 · 남의 코드를 읽는 법). 설계도와 진행 현황판은 `00-구성안.md`, 목차는 `README.md`.

- 작업·대화 중 새로운 개념을 설명하게 되면 해당 장에 반영하고, 마땅한 장이 없으면 `부록/인박스.md`에 "무슨 일이 있었나 → 원리는 뭔가 → 우리 코드 어디를 고쳤나" 형식으로 쌓는다. 반영했으면 사용자에게 한 줄로 알린다.
- **학습자료는 항상 현재 코드와 일치해야 한다.** 코드를 바꾸는 작업(기능 추가·구조 변경·파일 이동·이름 변경)을 하면, 각 부가 인용·설명하는 내용과 어긋나는 곳이 없는지 확인하고 어긋나면 같은 작업에서 해당 장도 갱신한다. 특히 **6부(코드 정독)는 파일 경로와 줄 번호를 직접 인용**하므로, 그 파일을 건드렸으면 반드시 같이 본다.
- `부록/프로젝트-지도.md`는 모든 폴더·파일의 역할·상호작용을 담은 지도다. 파일을 새로 만들거나·지우거나·역할이 바뀌면 지도도 같은 작업에서 갱신한다.
- 처음 등장하는 용어는 `부록/용어집.md`에 한 줄 정의를 추가한다 (한글 가나다순, 영문은 뒤쪽 알파벳순).
- 톤은 `docs/GCP-배포-가이드.md` 스타일: 아무것도 모른다고 가정, 개념 먼저 → 비유 → 실제 우리 코드 인용 → 직접 해보기. 개념은 반드시 우리 코드의 실제 파일·줄로 연결한다.
- 각 부 도입에는 `📖 교과서는 이렇게 말한다` / `🔨 우리는 이렇게 했다` / `⚖️ 그래서 뭘 얻고 뭘 잃었나` 세 칸 대조를 넣는다 (교재 전체를 꿰는 축).
- 프로그래밍 언어 개념(4부 영역)은 특히 공들인다 — 정의/호출 구분 같은 초심자 함정을 명시하고, 실습은 설치 없이 따라 할 수 있게.
- 새 장을 쓰면 `00-구성안.md`의 진행 현황표와 `README.md`의 상태 표시를 같은 작업에서 갱신한다.
