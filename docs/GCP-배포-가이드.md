...
## 📚 학습 노트: 여기서 처음 나온 명령들 해부

- **fallocate**: 파일에 디스크 공간을 즉시 할당. 옛 방식 `dd`(0을 실제로 2GB어치 쓰는 것, 몇 분 소요)와 달리 1초에 끝남
- **chmod와 권한 숫자**: `chmod`(change mode)의 숫자 3자리는 **소유자/그룹/그 외**의 권한이고, 각 자리는 읽기 4 + 쓰기 2 + 실행 1의 합입니다. `600`=주인만 읽고 쓰기, `644`=남은 읽기만(일반 파일 기본), `755`=남은 읽기+실행(폴더·프로그램), `700`=나만 전부. `ls -al` 맨 앞의 `-rw-------` 문자열이 이 권한의 표시입니다
- **mkswap**: 공간을 스왑용 구조로 "포맷"(USB를 FAT32로 포맷하는 것과 같은 개념의 스왑판)
- **swapon / swapoff**: 지금 즉시 스왑 편입/해제. 재부팅하면 풀리므로 fstab 등록이 따로 필요
- **echo**: 문자열을 표준 출력(화면)에 그대로 출력
- **`|` 파이프**: **왼쪽 명령의 출력을 오른쪽 명령의 입력으로 흘려보내는** 리눅스 핵심 문법. `docker compose logs | grep error`처럼 계속 만나게 됩니다
- **tee -a**: 입력을 화면과 파일에 동시에 기록 (-a는 append). `sudo echo '...' >> 파일`이 안 되는 이유는 `>>` 리다이렉션을 sudo가 아닌 **내 셸이** 수행하기 때문 — 그래서 "root 권한의 tee가 대신 써준다"는 관용구를 씁니다
- **fstab 한 줄의 6칸**: `장치, 마운트위치(스왑은 none), 형식(swap), 옵션(sw), dump 여부(0), 부팅 검사 순서(0)`. 일반 디스크라면 `/dev/sdb1 /data ext4 defaults 0 2` 형태
