...
/** db 자체 또는 진행 중인 트랜잭션 — 묶음 저장(API-117)이 여러 카드를 한 트랜잭션 안에서 저장할 때 넘긴다 */
 ...
 * tx를 주면 그 트랜잭션 안에서 실행한다 — better-sqlite3는 트랜잭션을 겹쳐 열 수 없어서, 바깥에서 묶을 때는 안에서 다시 열면 안 된다.
 */
export function saveTextContent(
  ...
  if (tx) return saveWith(tx, file, content, savedBy);
  return db.transaction((t) => saveWith(t, file, content, savedBy));
}
